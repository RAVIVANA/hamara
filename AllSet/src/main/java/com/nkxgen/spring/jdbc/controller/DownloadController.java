...
public class DownloadController {
	...
	public void downloadAuditData(HttpServletResponse response) {
		try (OutputStream outputStream = response.getOutputStream()) {
			...
			logger.info("Downloaded audit data as PDF");

			generator.generateAuditLogsPdf(auditLogsList, outputStream);
		} catch (Exception e) {
			logger.error("Failed to download audit data", e);
			e.printStackTrace();
		}
	}
	...
	public void downloadAccountData(HttpServletResponse response, @RequestParam("accountId") String accountid) {
		try (OutputStream outputStream = response.getOutputStream()) {
			...
			logger.info("Downloaded account transactions data as PDF for account ID: {}", accountId);

		} catch (Exception e) {
			logger.error("Failed to download account transactions data", e);
			e.printStackTrace();
		}
	}
	...
	public void downloadLoanData(HttpServletResponse response, @RequestParam("accountId") String accountid) {
		try (OutputStream outputStream = response.getOutputStream()) {
			...
			logger.info("Downloaded loan transactions data as PDF for account ID: {}", accountId);

		} catch (Exception e) {
			logger.error("Failed to download loan transactions data", e);
			e.printStackTrace();
		}
	}
